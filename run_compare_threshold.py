...
EBM_CURVE_DIR = OUTPUT_DIR / "threshold_models" / "threshold_ebm_curve"
...
def main() -> None:
    ...
    def predict_ebm_curve(fold: int, test_df: pd.DataFrame):
        model_path = EBM_CURVE_DIR / f"fold_{fold}.pkl"
        if not model_path.exists():
            raise FileNotFoundError(f"EBM curve threshold model not found: {model_path}")
        with open(model_path, "rb") as f:
            payload = pickle.load(f)
        model = payload["model"]
        feature_cols = payload["feature_cols"]
    ...
    results = [
        ...
        evaluate_model("threshold_ebm_curve", df, splits, predict_ebm_curve),
