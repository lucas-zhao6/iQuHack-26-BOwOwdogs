"""
Runtime predictor wrapper for GPR-based runtime modeling.
"""
...
from .runtime_model import GPRRuntimeModel


class GPRRuntimePredictor:
    """End-to-end runtime predictor with optional feature enrichment/selection."""

    def __init__(
        self,
        runtime_model: Optional[GPRRuntimeModel] = None,
    ) -> None:
        self.runtime_model = runtime_model or GPRRuntimeModel()
        ...
        self.threshold_feature = "log2"

    def _append_runtime_extras(self, X_rt: np.ndarray, X_enriched: np.ndarray) -> np.ndarray:
        # GPR runtime model already trains on the base feature matrix (including flags),
        # so we keep the feature set unchanged to match the fitted scaler shape.
        return X_rt
    ...
    def predict(self, X: np.ndarray, thresholds: np.ndarray) -> np.ndarray:
        ...
        if self.threshold_feature == "raw":
            thr_use = thresholds
        elif self.threshold_feature == "both":
            thr_use = np.column_stack(
                [np.log2(np.clip(thresholds, 1, 512)), thresholds]
            )
        else:
            thr_use = np.log2(np.clip(thresholds, 1, 512))

        return self.runtime_model.predict(X_rt, thr_use)
            ...
            pickle.dump(
                {
                    ...
                    "threshold_feature": self.threshold_feature,
                },
                f,
            )

    @classmethod
    def load(cls, path: str | Path) -> "GPRRuntimePredictor":
        path = Path(path)
        ...
        predictor.scaler_type = data.get("scaler_type", "standard")
        predictor.threshold_feature = data.get("threshold_feature", "log2")
        return predictor
