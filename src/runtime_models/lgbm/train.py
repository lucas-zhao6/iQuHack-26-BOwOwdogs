...
4. Saves trained models to outputs/runtime_models/runtime_lgbm
"""
...
MODEL_DIR = OUTPUT_DIR / "runtime_models" / "runtime_lgbm"
OPTUNA_TRIALS = 120
VALID_SIZE = 0.2
...
def tune_hyperparameters(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for fold, split in enumerate(splits):
            ...
            X_train_rt = append_runtime_extras(X_train_rt, X_train, enriched_names)
            X_test_rt = append_runtime_extras(X_test_rt, X_test, enriched_names)
            if X_val_rt is not None:
                X_val_rt = append_runtime_extras(X_val_rt, X_val, enriched_names)
...
def train_runtime_predictor(
    ...
    X_rt_train = append_runtime_extras(X_rt_train, X_enriched_train, enriched_feature_cols)
    if X_rt_val is not None:
        X_rt_val = append_runtime_extras(X_rt_val, X_enriched_val, enriched_feature_cols)
    ...
    predictor = RuntimePredictor(runtime_model=rt_model)
...
def main() -> None:
    ...
    print("=" * 70)
    print("PHASE 2: Runtime Model Training")
    print("=" * 70)
