#!/usr/bin/env python3
"""
Train the combined-architecture runtime model (runtime-only output).
"""
...
from src.threshold_models.lgbm.threshold_model import ThresholdModel
...
from src.evaluation.scoring import compute_runtime_metrics, threshold_to_idx
...
MODEL_DIR = OUTPUT_DIR / "runtime_models" / "runtime_combined"
OPTUNA_TRIALS = 120
VALID_SIZE = 0.2
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for fold, split in enumerate(splits):
            ...
            if X_val_thr is not None and can_use_threshold_eval_set(y_thr_train, y_thr_val):
                thr_eval = (X_val_thr, y_thr_val)
            thr_model.fit(X_train_thr, y_thr_train, eval_set=thr_eval)
            ...
            for flag_name in ["is_gpu", "is_double"]:
                if flag_name in enriched_names:
                    ...
                    extra_train.append(X_train[:, flag_idx])
                    extra_test.append(X_test[:, flag_idx])
                    if X_val is not None:
                        extra_val.append(X_val[:, flag_idx])

            X_train_rt = append_runtime_extras(X_train_rt, X_train, extra_train)
            X_test_rt = append_runtime_extras(X_test_rt, X_test, extra_test)
            if X_val_rt is not None:
                X_val_rt = append_runtime_extras(X_val_rt, X_val, extra_val)

            rt_model = RuntimeModel(lgb_params=rt_params)
            rt_model.fit(
                X_train_rt,
                y_time_train,
                y_thr_train,
            ...
            all_pred.extend([float(v) for v in pred_times])
...
def train_combined_runtime(
    ...
    X_rt_val = (
        selector.transform_runtime(X_enriched_val) if X_enriched_val is not None else None
    )
    ...
    train_proba = thr_model.predict_proba(X_thr_train)
    idxs = np.arange(train_proba.shape[1])
    train_expected = train_proba @ idxs
    train_entropy = -(train_proba * np.log(train_proba + 1e-9)).sum(axis=1)
    extra_train = [train_expected, train_entropy]
    ...
    if X_thr_val is not None:
        val_proba = thr_model.predict_proba(X_thr_val)
    ...
    for flag_name in ["is_gpu", "is_double"]:
        if flag_name in enriched_feature_cols:
            ...
            extra_train.append(X_enriched_train[:, flag_idx])
            if X_enriched_val is not None:
                extra_val.append(X_enriched_val[:, flag_idx])

    X_rt_train = append_runtime_extras(X_rt_train, X_enriched_train, extra_train)
    if X_rt_val is not None:
        X_rt_val = append_runtime_extras(X_rt_val, X_enriched_val, extra_val)

    rt_model = RuntimeModel(lgb_params=best_params["runtime_params"])
    rt_model.fit(
        X_rt_train,
        y_time_train,
        y_thr_train,
    ...
    predictor = CombinedRuntimePredictor(thr_model, rt_model)
    predictor.feature_columns = feature_cols
    predictor.enriched_feature_columns = enriched_feature_cols
    predictor.selected_feature_names_threshold = selector.get_selected_names(target="threshold")
    predictor.selected_feature_names_runtime = selector.get_selected_names(target="runtime")
...
def main():
    ...
    for fold, split in enumerate(splits):
        ...
        predictor = train_combined_runtime(
            df,
            X,
            y_threshold,
            y_total_time,
    ...
    full_predictor = train_combined_runtime(
        df,
        X,
        y_threshold,
        y_total_time,
