...
4. Saves trained models to outputs/combined_model
...
MODEL_DIR = OUTPUT_DIR / "combined_model"
OPTUNA_TRIALS = 120
OPTUNA_SPLITS = 5
VALID_SIZE = 0.2
...
def train_combined_model(
    ...
    """Train a CombinedPredictor for a single train/val split."""
    X_train = X[train_idx]
    X_val = X[val_idx] if val_idx.size > 0 else None
    ...
    combined = CombinedPredictor(thr_model, rt_model)
    combined.feature_columns = feature_cols
    combined.enriched_feature_columns = enriched_feature_cols
    combined.selected_feature_names_threshold = selector.get_selected_names(
        target="threshold"
    )
    combined.selected_feature_names_runtime = selector.get_selected_names(
        target="runtime"
    )
    combined.feature_selector = selector
    if aux_predictor is not None:
        combined.aux_predictor = aux_predictor
...
def main():
    ...
    # ── Step 4: Train per-fold models ──────────────────────────────────
    print("\n[4/5] Training per-fold models...")
    ...
    print("\n[5/5] Training full model on all data...")

    full_model = train_combined_model(
        df,
        X,
        y_threshold,
        y_total_time,
    ...
    enriched_feature_cols = (
        full_model.enriched_feature_columns if full_model.enriched_feature_columns else feature_cols
    )
